...
class NewBase(object):
    def __init__(self, name, mapper=None, inputs=None, other_mappers=None, mem_gb=None,
        ...
        self.name = name
        # dj TODO: I should think what is needed in the __init__ (I redefine some of rhe attributes
        # anyway)
        if inputs:
            # adding name of the node to the input name
            self._inputs = dict(("{}.{}".format(self.name, key), value)
        ...
        self._mapper = mapper
        # information about other nodes' mappers from workflow
        # (in case the mapper from previous node is used)
        self._other_mappers = other_mappers
        # create state (takes care of mapper, connects inputs with axes, so we can ask for specifc
        # element)
        ...
        # flag that says if value of state input should be printed in output and directories
        # (otherwise indices)
    ...
    def map(self, mapper, inputs=None):
        ...
        if inputs:
            inputs = dict(("{}.{}".format(self.name, key), value)
                          for (key, value) in inputs.items())
    ...
    def get_input_el(self, ind):
        ...
        for (from_node, from_socket, to_socket) in self.needed_outputs:
            ...
            if is_node(from_node) and is_current_interface(from_node.interface):
                file_from = self._reading_ci_output(node=from_node, dir_nm_el=dir_nm_el_from,
                                                    out_nm=from_socket)
            ...
            else:  # assuming I want to read the file (will not be used with current interfaces)
    ...
    def _reading_ci_output(self, dir_nm_el, out_nm, node=None):
        """used for current interfaces: check if output exists and return the path if it does"""
        if not node:
            node = self
...
class NewNode(NewBase):
    ...
    def run_interface_el(self, i, ind):
        ...
        if is_function_interface(self.interface):
            ...
            if not self.mapper:
                dir_nm_el = ""
            res = self.interface.run(inputs=inputs_dict,
                                     base_dir=os.path.join(os.getcwd(), self.workingdir),
                                     dir_nm_el=dir_nm_el)

        # TODO when join
        # if self._joinByKey:
        #     dir_join = "join_" + "_".join(["{}.{}".format(i, j)
        #                                    for i, j in list(state_dict.items())
        #                                    if i not in self._joinByKey])
        # elif self._join:
        #     dir_join = "join_"
    ...
    def _writting_results_tmp(self, state_dict, dir_nm_el, output):
        """temporary method to write results in files (this is usually part of a interface)"""
        if not self.mapper:
            dir_nm_el = ''
            ...
            for (i, ind) in enumerate(itertools.product(*self.state.all_elements)):
                ...
                if self.mapper:
                    if is_function_interface(self.interface):
                        ...
                        self._output[key_out][dir_nm_el] = (state_dict, (state_dict,
                                                                         self._reading_ci_output(
                                                                             dir_nm_el=dir_nm_el,
                                                                             out_nm=key_out)))
                else:
                    if is_function_interface(self.interface):
    ...
    def _check_all_results(self):
        ...
        for ind in itertools.product(*self.state.all_elements):
            ...
            for key_out in self.output_names:
                if is_function_interface(self.interface):
                    if not os.path.isfile(os.path.join(self.workingdir, dir_nm_el,
                                                       key_out+".txt")):
                        return False
                elif is_current_interface(self.interface):
...
class NewWorkflow(NewBase):
    def __init__(self, name, inputs=None, wf_output_names=None, mapper=None,  # join_by=None,
                 nodes=None, workingdir=None, mem_gb=None, cache_location=None, print_val=True,
                 *args, **kwargs):
        ...
        # list of (nodename, output name in the name, output name in wf) or
        # (nodename, output name in the name)
        # dj: using different name than for node, since this one it is defined by a user
        self.wf_output_names = wf_output_names

        # nodes that are created when the workflow has mapper
        # (key: node name, value: list of nodes)
        self.inner_nodes = {}
    ...
    @inputs.setter
    def inputs(self, inputs):
        self._inputs.update(dict(("{}.{}".format(self.name, key), value)
                            for (key, value) in inputs.items()))
    ...
    def get_output(self):
        # not sure, if I should collecto output of all nodes or only the ones that are used in
        # wf.output
        self.node_outputs = {}
                        ...
                        for (i, ind) in enumerate(itertools.product(*self.state.all_elements)):
                            if self.print_val:
                                ...
                                dir_nm_el = "_".join(["{}:{}".format(i, j)
                                                      for i, j in list(wf_inputs_dict.items())])
                            else:
                                wf_ind_dict = self.state.state_ind(ind)
                                dir_nm_el = "_".join(["{}:{}".format(i, j)
                                                      for i, j in list(wf_ind_dict.items())])
                            self._output[out_wf_nm][dir_nm_el] = \
                                self.node_outputs[node_nm][i][out_nd_nm]
                    else:
            ...
            for out in self.wf_output_names:
                ...
                if self.mapper:
                    for (i, ind) in enumerate(itertools.product(*self.state.all_elements)):
                        ...
                        dir_nm_el = "_".join(["{}:{}".format(i, j)
                                              for i, j in wf_inputs_dict.items()])
                        res_l = []
                        val_l = self._dict_tuple2list(self.output[key_out][dir_nm_el])
                        for val in val_l:
                            with open(val[1]) as fout:
                                logger.debug('Reading Results: file={}, st_dict={}'.format(val[1],
                                                                                           val[0]))
    ...
    def add(self, runnable, name=None, workingdir=None, inputs=None, output_names=None,
            mapper=None, mem_gb=None, print_val=True, out_read=False, **kwargs):
        if is_function(runnable):
            if not output_names:
                output_names = ["out"]
            interface = aux.FunctionInterface(function=runnable, output_nm=output_names,
                                              out_read=out_read)
            if not name:
                raise Exception("you have to specify name for the node")
            if not workingdir:
                workingdir = name
            node = NewNode(interface=interface, workingdir=workingdir, name=name, inputs=inputs,
                           mapper=mapper, other_mappers=self._node_mappers, mem_gb=mem_gb,
                           print_val=print_val)
            ...
            if not workingdir:
                workingdir = name
            node = NewNode(interface=runnable, workingdir=workingdir, name=name, inputs=inputs,
                           mapper=mapper, other_mappers=self._node_mappers, mem_gb_node=mem_gb,
                           output_names=output_names, print_val=print_val)
            ...
            if not workingdir:
                workingdir = name
            node = NewNode(interface=ci, workingdir=workingdir, name=name, inputs=inputs,
                           mapper=mapper, other_mappers=self._node_mappers, mem_gb_node=mem_gb,
    ...
    def preparing(self, wf_inputs=None, wf_inputs_ind=None):
        ...
        for node_nm, inp_wf, inp_nd in self.needed_inp_wf:
            ...
            if "{}.{}".format(self.name, inp_wf) in wf_inputs:
                node.state_inputs.update({"{}.{}".format(node_nm, inp_nd):
                                          wf_inputs["{}.{}".format(self.name, inp_wf)]})
                node.inputs.update({"{}.{}".format(node_nm, inp_nd):
                                    wf_inputs["{}.{}".format(self.name, inp_wf)]})
            else:
        ...
        for nn in self.graph_sorted:
            if self.print_val:
                ...
                dir_nm_el = "_".join(["{}:{}".format(i, j)
                                      for i, j in list(wf_inputs_ind.items())])
            if not self.mapper:
                dir_nm_el = ""
            ...
            try:
                for inp, (out_node, out_var) in self.connected_var[nn].items():
                    ...
                    # if there is no mapper provided, i'm assuming that mapper is taken from the
                    # previous node
